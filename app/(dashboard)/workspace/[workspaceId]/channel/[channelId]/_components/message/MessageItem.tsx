...
export function MessageItem({ message, currentUserId }: MessageItemProps) {
  ...
  const prefetchThread = useCallback(() => {
    const options = query.message.thread.list.queryOptions({
      input: {
        ...
        {isEditing ? (
          ...
          <>
            ...
            {message.imageUrl && (
              <div className="mt-3">
                <Image
                  ...
                  width={512}
                  height={512}
                  className="rounded-md r max-h-80 w-auto object-contain"
                />
              </div>
            )}
            {/* Reactions */}
            ...
            {message.replyCount > 0 && (
              <button
                type="button"
                className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-none focus-visible:ring-border cursor-pointer"
